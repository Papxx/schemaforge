...
## Was SchemaForge davon übernommen hat (Stand P2-05)
...
- **P2-05:** `McPrintActions.swapToHotbar` nutzt `InvUtils.quickSwap().fromId(hotbarIndex).to(inventoryIndex)` (ein `ContainerInput.SWAP`-Klick, Hotbar-Index als Button) – wie `MiddleClickExtra`. Nur wenn `player.containerMenu` ein `InventoryMenu` ist, weil `SlotUtils.indexToId` sonst in einen fremden Container zeigt.
